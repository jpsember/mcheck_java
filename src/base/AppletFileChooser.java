...
class AppletFileChooser implements FileChooser {
	...
	private DArray getFilteredList(PathFilter filter) {
		DArray a = new DArray();
		SimFile[] ls = EmulationConsole.appletFileList().getFiles();
	 ...
	 */
	public String doWrite(String prompt, String path, PathFilter filter) {
		DArray a = getFilteredList(filter);
		String out = myListDialog.showDialog(null, null, prompt, a, findString(
				path, a), true);
...
class myListDialog extends JDialog implements ActionListener,
	...
	private myListDialog(Frame frame, Component locationComp, String title,
		...
		list = new JList(items.toArray(true));
		list.setFont(WorkTextArea.monoFont());
...
class ApplicationFileChooser implements FileChooser {
	/**
	 * Get name of file to open
	 * 
	 * @param path :
