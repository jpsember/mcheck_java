 ...
 *
 * - : hides file
 * ! : file is a program (implies hidden)
 */
public class AppletFileList {
...
//    System.out.println("AppletFileList: adding file "+sf.debug());
  ...
  /**
   * Read and process 'appletdir.txt' from the JAR file.
   * Processes each entry as the name of a file that is
   * assumed to be in the JAR.
   * If the name starts with '-', its hidden flag will be set.
   * If it starts with '!', it will be loaded into the stdin window (if one exists).
   */
  void processDirFile() throws IOException {
    ...
    // attempt to open stream from resource directly,
    // since "appletdir.txt" will not be in the AppletFileList
    InputStream is = null;
    try {
      is = Streams.openResource("appletdir.txt");
    }
...
if (db) Streams.out.println(" read line "+n);
          ...
          if (db) {
            Streams.out.println(Scanner.chomp(n));
          }
          if (s.peekChar() == '#') {
            continue;
          }
