...
public class CmdArgs {
  ...
  public CmdArgs(String[] args, String defaults, String helpMsg) {
    ...
    if (defaults != null) {
      ...
      Scanner s = new Scanner(defaults);
      while (true) {
//        Token tk = s.read();
//
        s.readWS();
...
//        }
        ...
        if (db) {
          System.out.println("CmdArgs, scanning " + Scanner.debug(arg));
        }
        if (arg.equals("!!")) {
          modeDef = true;
        }
...
//  }
  ...
  public String nextValue() {
    ...
    if (!nextIsValue()) {
      StringBuffer sb = new StringBuffer("Missing value in arguments");
      if (lastOption != null) {
  ...
  private static boolean isOption(String s) {
    ...
    do {
      ...
      if (s.length() > 1
          && (
              s.charAt(1) != '-'
              && !Scanner.isIdentifierStart(s.charAt(1)))) {
        break;
      }
      out = true;
    }
