...
//      ;
  ...
  public static String chomp(StringBuffer s) {
   ...
   * @param sb : destination StringBuffer; null to create one
   * @return StringBuffer
   */
  public static StringBuffer convert(char c, char delim, StringBuffer sb) {
    if (sb == null) {
      sb = new StringBuffer();
    }
    switch (c) {
        ...
        case ESC: {
          ...
          switch (c) {
            ...
            case 'u':
              ...
              sb.append( (char) Scanner.parseHex(orig, i + 1, 4));
              i += 4;
              break;
            case 'n':
              sb.append('\n');
...
//  final char q = '"';
   ...
   */
  public Scanner(Reader reader, String sourceDescription, DFA dfa, int skipType) {
    this(dfa, skipType);
    include(reader, sourceDescription);
  }

  public Scanner(DFA dfa, int skipType) {
    this.dfa = dfa;
...
//    System.out.println("Scanner, setTrace "+t);
...
//          ri.closed = true;
  ...
  private StringBuffer echoBuffer = new StringBuffer();
//private int echoLineNumber;
...
//      echoLineNumber = 0;
  ...
  public static DArray scanSource(Reader src,
    ...
    Scanner scan = new Scanner(src, srcDescription, dfa, skipType);
    while (true) {
      Token t = scan.read();
      if (t.eof()) {
        break;
...
//    int mask = 0xf << shift;
   ...
   * @return int
   */
  public static int parseHex(String s, int offset, int length) {
    int val = 0;
